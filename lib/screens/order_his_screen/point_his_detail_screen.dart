...
class PointHisDetailScreen extends StatefulWidget {
  final PointHistoryData orderData;
  final Color orderColor;
  const PointHisDetailScreen({
    super.key,
  ...
  @override
  State<PointHisDetailScreen> createState() => _PointHisDetailScreenState();
}

class _PointHisDetailScreenState extends State<PointHisDetailScreen> {
  final OrderHisController controller = Get.find<OrderHisController>();
  late bool isPayment = false;
  @override
  void initState() {
    setState(() {
      if (widget.orderData.order!.payment == null) {
        isPayment = false;
      } else {
        isPayment = true;
      }
      ...
      body: ListView(
        ...
        children: [
                ...
                children: [
                  Column(
                    ...
                    children: [
                      ...
                      Text(
                        widget.orderData.points.toString() + " Points".tr,
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      //   ),
                      // ),
                      if (widget.orderData.order!.refundDate!.isEmpty &&
                          widget.orderData.order!.refundMessage!.isEmpty)
                        Text(
                          // orderStatusText,
                          orderText(widget.orderData.order!.status.toString()),
                          style: context.titleMedium.copyWith(
                            color: orderColors(
                              widget.orderData.order!.status.toString(),
                            ),
                          ),
                        )
                      else
          ...
          ListView.builder(
            ...
            itemCount: widget.orderData.order!.orderItem!.length,
            itemBuilder: (_, index) {
              OrderItem orderItem = widget.orderData.order!.orderItem![index];
              return InkWell(
                onTap:
                    () => {
          ...
          Visibility(
            ...
            child: Column(
              ...
              children: [
                ...
                if (widget.orderData.order!.payment != null)
                  SelectPaymentWidget(
                    paymentData: widget.orderData.order!.payment!,
                  ),
                SizedBox(height: Dimesion.width5),
                if (widget.orderData.order!.payment != null)
                  OrderImageWidget(
                    imgUrl: widget.orderData.order!.paymentPhoto.toString(),
                    title: "Order Slip",
                  ),
              ],
            ),
          ),
          if (isPayment == false && widget.orderData.order!.payment == null)
            RowTextWidget(title: "Payment Type".tr, val: "Cash On Delivery"),

          Divider(color: Colors.grey[300]),
          RowTextWidget(title: "Name", val: widget.orderData.order!.name ?? ""),
          RowTextWidget(
            title: "Phone Number",
            val: widget.orderData.order!.phone ?? "",
          ),
          RowTextWidget(
            title: "Address",
            val: "${widget.orderData.order!.address}",
          ),
          Divider(color: Colors.grey[300]),
          RowTextWidget(
            title: "Sub Total",
            val:
                "${DataConstant.priceFormat.format(int.parse(widget.orderData.order!.subTotal.toString()))} MMK",
          ),
          RowTextWidget(
            title: "Delivery Fees",
            val:
                "${DataConstant.priceFormat.format(int.parse(widget.orderData.order!.deliveryFee.toString()))} MMK",
          ),
          RowTextWidget(
            title: "Grand Total",
            val:
                "${DataConstant.priceFormat.format(int.parse(widget.orderData.order!.grandTotal.toString()))} MMK",
          ),
        ],
      ),
    );
